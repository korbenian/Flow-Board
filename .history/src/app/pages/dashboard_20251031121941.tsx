...
export default function Dashboard () {
  ...
  const metrics = [
    { id: 1, title: t('tasksCreated'), value: taskCount, icon: ShoppingBag },
    { id: 2, title: t('totalUsers'), value: count, icon: Users },
    { id: 3, title: t('tasksDone'), value: tcount, icon: CheckCircle },
    {
      id: 4,
      title: t('disciplineStreak'),
      value: actualStreak !== null ? `${actualStreak} ${t('days')}` : '—',
      icon: Flame,
      ...
      <div className='flex-1 p-6 space-y-6'>
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4'>
          {metrics.map(m => (
            <div
              key={m.id}
              className='dark:border-amber-50 border-2 shadow rounded-2xl p-4 flex items-center gap-3'
            >
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
          <div className='dark:border-amber-50 border-2 shadow rounded-2xl p-4 lg:col-span-2'>
          ...
          <div className='dark:border-amber-50 border-2 shadow rounded-2xl p-4'>
            <h2 className='text-lg font-semibold mb-4'>
              {t('recentActivities')}
            </h2>
            <ul className='space-y-3'>
              {activities.length > 0 ? (
                activities.map(a => (
                  <li key={a.id} className='text-sm dark:text-amber-50'>
                    • {a.text}
                  </li>
                ))
              ) : (
